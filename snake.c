...
float getJoystickInput(uint32_t, int);
void setupJoystick(uint32_t);
...
struct apple spawnApple(int cubeGrid[8][8][8]){
    ...
    for (int x = 0; x < 8; ++x){
        for (int y = 0; y < 8; ++y){
            for (int z = 0; z < 8; ++z){
                if (cubeGrid[x][y][z] == 0) {
                    candidates[candCount][0] = x;
                    candidates[candCount][1] = y;
                    candidates[candCount][2] = z;
                    candCount++;
                }
            }
        }
    ...
    int ri = rand() % candCount; // Gets a random valid apple position
...
void setupJoystick(uint32_t adc){
    ...
    adc_power_off(adc);  //Turn off ADC register whist we set it up

    adc_set_clk_prescale(adc, ADC_CCR_CKMODE_DIV1);  //Setup a scaling, none is fine for this
    adc_disable_external_trigger_regular(adc);   //We don't need to externally trigger the register...
    adc_set_right_aligned(adc);  //Make sure it is right aligned to get more usable values
    adc_set_sample_time_on_all_channels(adc, ADC_SMPR_SMP_61DOT5CYC);  //Set up sample time
    adc_set_resolution(adc, ADC_CFGR1_RES_12_BIT);  //Get a high resolution so you can detect small changes in the joystick

    adc_power_on(adc);  //Finished setup, turn on ADC register 1
}
